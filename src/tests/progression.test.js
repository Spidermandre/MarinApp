...
import { DEFAULT_INCREMENTS, parseRepRange, roundToIncrement, suggestCardioAdjust, suggestDuration, suggestWeight, } from '../lib/progression';
...
const entry = (sessionNumber, prescribedReps, sets) => ({
    sessionNumber,
    prescribedReps,
...
describe('doppia progressione', () => {
    ...
    it('rispetta l’incremento per tipo di attrezzo', () => {
        ...
        expect(suggestWeight(goblet, item({ exerciseId: 'goblet-squat', reps: '12-15' }), h).weightKg).toBe(22);
        expect(suggestWeight(goblet, item({ exerciseId: 'goblet-squat', reps: '12-15' }), h, {
            ...DEFAULT_INCREMENTS,
            dumbbellStepKg: 1,
        }).weightKg).toBe(21);
    });
