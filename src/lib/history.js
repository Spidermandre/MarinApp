...
export function prescribedRepsFor(sessionNumber, exerciseId) {
    try {
        const w = workoutForSession(sessionNumber);
        const item = w.strength.find((s) => s.exerciseId === exerciseId) ??
...
export function buildHistory(exerciseId, sessions, sets) {
...
export function lastTimeLabel(history) {
